...
module.exports=async function(req,res){
 ...
 try{
  ...
  const action=String(req.body?.action||'monthly-response');
  ...
  if(action==='delete-replacement'){
   await sb(`replacement_requests?id=eq.${encodeURIComponent(id)}`,{method:'DELETE',prefer:'return=minimal'});
   return json(res,200,{ok:true});
  }

  if(action==='delete-weekly'){
   await sb(`weekly_checkins?id=eq.${encodeURIComponent(id)}`,{method:'DELETE',prefer:'return=minimal'});
   return json(res,200,{ok:true});
  }

  if(action==='delete-monthly'){
   await sb(`monthly_reviews?id=eq.${encodeURIComponent(id)}`,{method:'DELETE',prefer:'return=minimal'});
   return json(res,200,{ok:true});
  }

  if(action==='replacement-response'){
   const rows=await sb(`replacement_requests?id=eq.${encodeURIComponent(id)}`,{
    method:'PATCH',
    prefer:'return=representation',
    body:{trainer_reply:String(req.body?.feedback||''),status:'done'}
   });
   return json(res,200,{request:rows?.[0]});
  }

  if(action==='weekly-response'){
   const rows=await sb(`weekly_checkins?id=eq.${encodeURIComponent(id)}`,{
    method:'PATCH',
    prefer:'return=representation',
    body:{trainer_reply:String(req.body?.feedback||''),status:'done'}
   });
  ...
  const rows=await sb(`monthly_reviews?id=eq.${encodeURIComponent(id)}`,{
   method:'PATCH',
   prefer:'return=representation',
   body:{trainer_feedback:String(req.body?.feedback||''),status:'done',responded_at:new Date().toISOString()}
  });
  return json(res,200,{review:rows?.[0]});

 }catch(e){
  console.error('ADMIN_REVIEW_ERROR',e);
  return json(res,403,{error:e.message||'Admin response error'});
 }
};
