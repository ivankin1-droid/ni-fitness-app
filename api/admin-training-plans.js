...
module.exports=async function handler(req,res){
 ...
 try{
  ...
  if(action==='create'){
   ...
   await sb(`training_plans?telegram_id=eq.${encodeURIComponent(telegramId)}&is_active=eq.true`,{
    method:'PATCH',
    prefer:'return=minimal',
    body:{is_active:false}
   });

   const rows=await sb('training_plans',{
    method:'POST',
    prefer:'return=representation',
    body:[{telegram_id:telegramId,plan_name:planName,trainer_comment:trainerComment,days,is_active:true}]
   });
  ...
  if(action==='delete'){
   ...
   const owned=await sb(`training_plans?id=eq.${encodeURIComponent(id)}&telegram_id=eq.${encodeURIComponent(telegramId)}&select=id&limit=1`);
   if(!Array.isArray(owned)||!owned[0])return json(res,404,{error:'План не найден.'});

   await sb(`training_plans?id=eq.${encodeURIComponent(id)}&telegram_id=eq.${encodeURIComponent(telegramId)}`,{
    method:'DELETE',
    prefer:'return=minimal'
   });
  ...
  console.error('ADMIN_TRAINING_ERROR',e);
  return json(res,403,{error:e.message||'Training plan error'});
 }
};
